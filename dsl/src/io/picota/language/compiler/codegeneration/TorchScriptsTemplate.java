...
public class TorchScriptsTemplate extends Template {
	...
	public List<Rule> ruleSet() {
		...
		rules.add(rule().condition(trigger("importvariable")).output(literal("import ")).output(placeholder(new String[]{"container"}, "name", "camelcase", "firstLowercase")).output(literal(".")).output(placeholder("viewPoint")).output(literal(".")).output(placeholder("name")));
		rules.add(rule().condition(trigger("callvariable")).output(placeholder(new String[]{"container"}, "name", "camelcase", "firstLowercase")).output(literal(".")).output(placeholder("viewPoint")).output(literal(".")).output(placeholder("name")).output(literal(".train(variables(dt, \"")).output(placeholder("viewPoint")).output(literal("_")).output(placeholder("name")).output(placeholder("timeHorizon")).output(literal("\"), modeldir)")));
		rules.add(rule().condition(trigger("evaluate")).output(literal("print(\"")).output(placeholder("viewPoint")).output(literal("_")).output(placeholder("name")).output(literal("\t{}\".format(KanEvaluator.eval(sys.argv[1] + \"/")).output(placeholder(new String[]{"container"}, "name")).output(literal("/")).output(placeholder("viewPoint")).output(literal("/")).output(placeholder("name")).output(placeholder("timeHorizon")).output(literal(".bin\", tensor)))")));
		rules.add(rule().condition(allTypes("variable")).output(literal("import torch\nimport os\nfrom torch import nn\nfrom InputDataset import InputDataset\nfrom kan.KanTrainer import KanTrainer\n\ndef input_variables(dict, output):\n\treturn {key: valor for key, valor in dict.items() if key != output}\n\ndef train(dt, modeldir):\n\tmodel_path = modeldir + \"/")).output(placeholder("viewPoint")).output(literal("/")).output(placeholder("name")).output(placeholder("timeHorizon")).output(literal(".bin\"\n\toutput_variable = \"")).output(placeholder("viewPoint")).output(literal("_")).output(placeholder("name")).output(placeholder("timeHorizon")).output(literal("\"\n\tos.makedirs(modeldir + \"/")).output(placeholder("viewPoint")).output(literal("/\", exist_ok=True)\n\tdevice = \"cpu\"\n\tbatch_size = 2\n\tepochs = 5\n\tlr = 0.001\n\tloss_fn = nn.MSELoss()\n\tvalidation_loss_fn = nn.L1Loss()\n\ttest_proportion = 0.2\n\tvalidation_proportion = 0.16\n\tdataset = InputDataset(dt, output_variable)\n\tmodel, loss, features = (KanTrainer(input_variables(dt, output_variable), output_variable, batch_size, epochs, device, test_proportion, validation_proportion, lr, loss_fn, validation_loss_fn)\n    \t\t\t\t.train(dataset))\n\ttorch.save(model.state_dict(), model_path)\n\tprint(f\"{")).output(placeholder(new String[]{"context"}, "name")).output(literal("}\\t{output_variable}\t{loss}\t\" + \"\t\".join(features.keys()))")));
		return rules;
	}
