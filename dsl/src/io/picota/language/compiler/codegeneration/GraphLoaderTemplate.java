...
public class GraphLoaderTemplate extends Template {
	...
	public List<Rule> ruleSet() {
		...
		rules.add(rule().condition(all(allTypes("digitalTwin"), trigger("method"))).output(literal("private static void ")).output(placeholder("name")).output(literal("(Datalake datalake, Datamart master) {\n\tSensor ")).output(placeholder("name")).output(literal("_Sensor = datalake.graph().create(\"")).output(placeholder("name")).output(literal("\").sensor();\n\t")).output(expression().output(placeholder("variable").multiple("\n"))).output(literal("\n\tEntity ")).output(placeholder("name")).output(literal("_Entity = master.create(\"")).output(placeholder("name")).output(literal("\").entity();\n\tDatalake.Tank.Measurement ")).output(placeholder("name")).output(literal("_Tank = datalake.create().tank().asMeasurement(")).output(placeholder("name")).output(literal("_Sensor, ")).output(placeholder("period")).output(literal(", Scale.")).output(placeholder("scale", "firstUpperCase")).output(literal(");\n\tmaster.create(\"")).output(placeholder("name")).output(literal("_Timeline\").timeline(")).output(placeholder("name")).output(literal("_Entity).asRaw(")).output(placeholder("name")).output(literal("_Tank);\n}")));
		rules.add(rule().condition(trigger("variable")).output(placeholder(new String[]{"container"}, "name")).output(literal("_Sensor.create(\"")).output(placeholder("")).output(literal("\").magnitude(\"")).output(placeholder("")).output(literal("\");")));
		return rules;
	}
