...
public class GraphLoaderTemplate extends Template {
	...
	public List<Rule> ruleSet() {
		...
		rules.add(rule().condition(all(allTypes("numeric"), trigger("variable"))).output(placeholder(new String[]{"container"}, "name")).output(literal("_Sensor.create(\"")).output(placeholder("name")).output(literal("\").magnitude(\"")).output(placeholder("name")).output(literal("\").create(\"type\").attribute(\"Numeric\");")));
		rules.add(rule().condition(all(allTypes("numeric"), trigger("variable"))).output(placeholder(new String[]{"container"}, "name")).output(literal("_Sensor.create(\"")).output(placeholder("name")).output(literal("\").magnitude(\"")).output(placeholder("name")).output(literal("\").create(\"type\").attribute(\"Cyclic\");")));
		rules.add(rule().condition(trigger("variable")).output(placeholder(new String[]{"container"}, "name")).output(literal("_Sensor.create(\"")).output(placeholder("name")).output(literal("\").magnitude(\"")).output(placeholder("name")).output(literal("\");")));
		return rules;
	}
