...
public class BackendWebServer {
	...
	public BackendWebServer(Config config, UiCommands commands) {
	...
	private static void writeError(Context ctx, int status, String code, String message, Map<String, Object> details) {
		ctx.status(status).json(new io.picota.backend.control.ui.Error(code, message, details == null ? Map.of() : Map.copyOf(details)));
	}
}
