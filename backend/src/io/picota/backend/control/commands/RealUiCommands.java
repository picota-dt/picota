...
import io.picota.backend.model.TrainingJobAggregate;
...
public class RealUiCommands implements UiCommands {
	...
	public RealUiCommands() {
		this(null);
	}

	public RealUiCommands(ModelPersistence persistence) {
	...
	private void seedDemoState() {
		...
		usersById.put(demoUser.id(), new StoredUser(demoUser, UiCommandFixtures.DEFAULT_PASSWORD));
		userIdByEmail.put(demoUser.email().toLowerCase(Locale.ROOT), demoUser.id());
		ConcurrentMap<String, DigitalTwin> twins = new ConcurrentHashMap<>();
		for (DigitalTwin twin : UiCommandFixtures.demoTwins()) {
			twins.put(twin.id(), UiCommandFixtures.copyTwin(twin));
		}
		twinsByUser.put(demoUser.id(), twins);
	}
	...
	private boolean loadPersistedState() {
		...
		ApplicationModel model = persistence.loadModel().orElse(null);
		...
		for (TwinAggregate aggregate : model.twins()) {
			...
			twinsByUser.computeIfAbsent(aggregate.ownerUserId(), ignored -> new ConcurrentHashMap<>())
					.put(aggregate.twin().id(), aggregate.twin());
		}
		for (TrainingJobAggregate aggregate : model.trainingJobs()) {
			...
			trainingJobs.put(aggregate.job().jobId(), aggregate.job());
	...
	private ApplicationModel exportModel() {
		List<UserAccount> users = usersById.values().stream()
				.map(stored -> UserAccount.fromUiUser(stored.user(), stored.password()))
		...
		List<TwinAggregate> twins = twinsByUser.entrySet().stream()
				.flatMap(ownerEntry -> ownerEntry.getValue().values().stream()
						.map(twin -> new TwinAggregate(ownerEntry.getKey(), twin)))
				.sorted(Comparator.comparing(aggregate -> aggregate.twin().id()))
				.toList();

		List<TrainingJobAggregate> jobs = trainingJobs.values().stream()
				.map(job -> new TrainingJobAggregate(trainingJobOwnerById.get(job.jobId()), job))
				.sorted(Comparator.comparing(aggregate -> aggregate.job().jobId()))
				.toList();

		return new ApplicationModel(users, sessions, twins, jobs);
	}
