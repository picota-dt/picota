...
public class TorchScriptsTemplate extends Template {
	...
	public List<Rule> ruleSet() {
		...
		rules.add(rule().condition(trigger("importvariable")).output(literal("import ")).output(placeholder(new String[]{"container"}, "name", "camelcase", "firstLowerCase")).output(literal(".")).output(placeholder("name", "normalize")));
		...
		rules.add(rule().condition(trigger("evaluate")).output(literal("loader = DatasetLoader(sys.argv[2]+\"/")).output(placeholder(new String[]{"container"}, "name")).output(literal("_")).output(placeholder("name")).output(literal(".jsonl\")\ndata = loader.load()\nprint(\"")).output(placeholder(new String[]{"container"}, "name")).output(literal("\t")).output(placeholder("name")).output(placeholder("timeHorizon")).output(literal("\t{}\".format(KanEvaluator.eval(sys.argv[1] + \"/")).output(placeholder(new String[]{"container"}, "name")).output(literal("/")).output(placeholder("name")).output(placeholder("timeHorizon")).output(literal(".bin\",\n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\tdata,\n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t  \tloader.get_input_variables(),\n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t  \tloader.get_means(),\n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t  \tloader.get_stds(),\n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t  \tloader.get_lookback())))")));
