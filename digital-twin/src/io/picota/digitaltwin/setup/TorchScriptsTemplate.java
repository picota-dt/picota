...
public class TorchScriptsTemplate extends Template {
	...
	public List<Rule> ruleSet() {
		...
		rules.add(rule().condition(trigger("callvariable")).output(literal("if not os.path.exists(source + \"/")).output(placeholder(new String[]{"container"}, "name")).output(literal("_")).output(placeholder("name")).output(literal("\" + \".jsonl\"):\n\tprint(\"")).output(placeholder(new String[]{"container"}, "name")).output(literal("\t")).output(placeholder("name")).output(literal("\tNaN\")\n\treturn\nloader = DatasetLoader(source + \"/")).output(placeholder(new String[]{"container"}, "name")).output(literal("_")).output(placeholder("name")).output(literal("\" + \".jsonl\")\n")).output(placeholder(new String[]{"container"}, "name", "camelcase", "firstLowercase")).output(literal(".")).output(placeholder("name")).output(literal(".train(loader.load(), loader.get_input_variables(), loader.get_means(), loader.get_stds(), modeldir)")));
