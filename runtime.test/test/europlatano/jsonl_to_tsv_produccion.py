...
def _aggregate_output_tsv_by_albaran(
        output_tsv: Path,
        tmp_dir: str | None = None,
) -> tuple[int, int]:
    if not output_tsv.exists():
        raise RuntimeError(f"Output TSV not found for Albaran aggregation: {output_tsv}")

    tmp_kwargs: dict[str, Any] = {"prefix": "produccion_albaran_", "suffix": ".tsv", "delete": False}
    if tmp_dir:
        tmp_kwargs["dir"] = tmp_dir
    ...
    try:
        with output_tsv.open("r", encoding="utf-8", newline="") as source, aggregated_path.open(
            ...
            try:
                ...
                raise RuntimeError("Output TSV is empty; cannot aggregate by Albaran.") from error

            if "Albaran" not in header:
                raise RuntimeError("Column 'Albaran' not found in output TSV; cannot aggregate by Albaran.")

            albaran_index = header.index("Albaran")
            kilos_index = header.index("Kilos") if "Kilos" in header else None

            grouped: dict[str, dict[str, Any]] = {}
            missing_albaran_counter = 0
            ...
            for row in reader:
                ...
                input_rows += 1
                albaran_value = _normalize_join_key(row[albaran_index])
                if not albaran_value:
                    missing_albaran_counter += 1
                    group_key = f"__missing_albaran_{missing_albaran_counter}"
                else:
                    group_key = albaran_value
                ...
                if state is None:
                    ...
                    if albaran_value:
                        state["row"][albaran_index] = albaran_value
...
def _second_pass(
    ...
    return projected_unique_rows, projected_duplicates, removed_non_positive_kilos
...
def main() -> int:
    ...
    try:
        ...
        force_drop_columns = {
            ...
            "dir",
            "racha",
            "Territory.Temperature:Average",
            "Territory.Temperature:Min",
            "Territory.Temperature:Max",
            "Territory.Precipitation",
            "Territory.Humidity",
            "Territory.WindSpeed:Average",
            "Territory.WindDirection",
            "Territory.WindSpeed:Max",
        }
        keep_columns = _reorder_columns(keep_columns)
        projected_unique_rows, projected_duplicates, removed_non_positive_kilos = _second_pass(
            unique_jsonl=unique_jsonl,
