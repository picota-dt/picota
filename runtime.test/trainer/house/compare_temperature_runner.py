...
from kan.MetamorphicCatalog import (
    build_house_temperature_rule_specs,
...
def infer_feature_layout(input_variables: list[str], sample_batch: dict) -> tuple[int, list[str], int]:
    t_count = int(sample_batch["t"].shape[-1]) if "t" in sample_batch and sample_batch["t"].dim() > 1 else 0
    numerical_count = (
        int(sample_batch["numerical_t_features"].shape[-1])
        if "numerical_t_features" in sample_batch and sample_batch["numerical_t_features"].dim() > 1
        else 0
    )
    categorical_count = (
        int(sample_batch["categorical_t_features"].shape[-1])
        if "categorical_t_features" in sample_batch and sample_batch["categorical_t_features"].dim() > 1
        else 0
    )
    numerical_names = list(input_variables)[t_count:t_count + numerical_count]
    return t_count, numerical_names, categorical_count
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare baseline vs composite metamorphic training on House temperature."
    )
    parser.add_argument(
        "--jsonl",
        type=Path,
        default=ROOT / "data" / "house" / "House_Temperature.jsonl",
        help="Path to House_Temperature.jsonl",
    )
    parser.add_argument(
        "--md",
        type=Path,
        default=ROOT / "data" / "house" / "House_Temperature.md",
        help="Path to House_Temperature.md",
    )
    ...
    if split.train_items:
        ...
        _, numerical_t_feature_names, categorical_t_feature_count = infer_feature_layout(
